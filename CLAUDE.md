...
| Strava **Fase 5** (Seguridad) — PR1 check-in/S.O.S. por SMS | ✅ Hecho + validado en campo (2026-06-26). `safety/` + `shared/utils/trustedContacts` (AsyncStorage, solo en el dispositivo) + `application/safety/buildLocationShare` (mejor fix + link de Maps). **Offline** (SMS por red de voz; GPS sin datos). `expo-sms`. Entrada en Perfil y atajo 🛡️ en grabación |
| Strava **Fase 5** (Seguridad) — PR2 seguimiento en vivo (link "sígueme") | ✅ Implementado (2026-06-26, **in-app + Supabase**, pendiente validación de campo). Tabla `nk_live_sessions` (última posición in-place) + **RLS solo-dueño** + RPC `nk_get_live_session(token)` **SECURITY DEFINER** (capacidad por token, sin enumeración) + **trigger TTL 12 h** en servidor. Emisor: toggle "Compartir en vivo" en grabación → sube cada ~10 s (`useTracking`) → enlace por el mismo SMS. Visor: la **misma app** (`/seguir/[token]` + pegar enlace `/seguir`), polling 10 s. `application/live/liveShareUseCases`, `stores/liveShareStore`. Pasó `security-review`. **Requiere aplicar `supabase/schema.sql`.** PR3 (visor web) pendiente |
...
- [ ] **Pendientes (lista única y ordenada): ver `docs/STRAVA_ROADMAP.md` §"Pendientes por implementar"** — quedan: 🟡 cliente (detalle público con elevación interactiva, *snap* del planner, métricas DEM/nombres de zona), 🟢 backend **Fase 5 PR3 (visor web en vivo)** y **Fase 6 (social)**, y validación de campo (GPS reposo, auto-pausa, **seguimiento en vivo PR2**).
