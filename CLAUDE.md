...
| Strava **Fase 5** (Seguridad) — PR1 check-in/S.O.S. | ✅ Hecho + validado en campo (2026-06-26). `safety/` + `shared/utils/trustedContacts` (AsyncStorage, solo en el dispositivo) + `application/safety/buildLocationShare` (mejor fix + link de Maps). **Offline** (SMS por red de voz; GPS sin datos). `expo-sms`. Compartir multicanal (WhatsApp/SMS/copiar/sistema) vía `components/ui/ShareMessageSheet`. Entrada en Perfil y atajo 🛡️ en grabación |
| Strava **Fase 5** (Seguridad) — PR2 seguimiento en vivo (link "sígueme") | ✅ Hecho + validado en campo (2026-06-26, **in-app + Supabase**). Tabla `nk_live_sessions` (última posición in-place) + **RLS solo-dueño** + RPC `nk_get_live_session(token)` **SECURITY DEFINER** (capacidad por token, sin enumeración) + **trigger TTL 12 h** en servidor. Emisor: toggle "Compartir en vivo" en grabación → sube cada ~10 s (`useTracking`) → enlace por WhatsApp/SMS/copiar/sistema (`ShareMessageSheet`). Visor: la **misma app** (`/seguir/[token]` + pegar enlace `/seguir`), polling 10 s. `application/live/liveShareUseCases`, `stores/liveShareStore`. Pasó `security-review`. **Requiere aplicar `supabase/schema.sql`.** **PR3 (visor web) ✅ hecho + validado 2026-06-28**: el mismo App Link, sin la app, cae al visor de la landing (`404.astro`, mapa Leaflet/OSM + polling del RPC con la publishable key/`anon`); `GRANT EXECUTE … TO anon` en la RPC |
| **Observabilidad** — reporte de bug in-app + analítica de uso (in-house) | ✅ Hecho + **schema aplicado y validado** (2026-06-26). Decisión: **NO** Google/Firebase (privacidad de app de ubicación + datos propios). Bug report: `report-bug.tsx` + `application/feedback/submitBugReport` → `nk_bug_reports` + bucket privado `nk-bug-shots` (entrada en Perfil). Analítica: `infrastructure/services/AnalyticsService` (`trackEvent`/cola offline AsyncStorage/flush por lotes/opt-out) → `nk_events`; `ScreenViewTracker` en `_layout` + ~8 eventos clave. **Privacidad dura: NUNCA GPS/PII**; pantalla = patrón de segmento; toggle opt-out en Perfil. **Dashboard: proyecto Astro propio** en `D:\projects\nan-kamay-dashboard` (SSR + Chart.js; lee `nk_events`/`nk_bug_reports` con rol Postgres solo-lectura `metabase_ro` vía **Session pooler**, no la directa); Metabase como alternativa. Guía: `docs/ANALYTICS.md`. Pendiente solo generar datos de uso real. Heatmaps reales diferidos (privacidad) |

**Antes de tocar sync, backend o esquema:** lee `docs/VALIDATION.md`.
**Antes de tocar analítica/observabilidad:** lee `docs/ANALYTICS.md` (reglas de privacidad + catálogo de eventos).
...
nk_live_sessions (id UUID PK, user_id UUID FK auth.users, token UNIQUE, route_id [sin FK: ruta borrador],
        owner_name, status('active'|'ended'), last_lat/lon/altitude/accuracy/speed/at, distance_meters,
        started_at, ended_at, expires_at, created_at)   ← seguimiento en vivo (Fase 5 PR2)
```
...
- **`nk_live_sessions` (Fase 5 PR2):** RLS **solo-dueño**; el visor lee SOLO vía la función
  `nk_get_live_session(token)` **SECURITY DEFINER** (capacidad por token exacto, sin enumeración, no
  expone `user_id`/`token`) — `REVOKE FROM PUBLIC` + `GRANT a authenticated`. Un **trigger** fuerza el
  TTL de 12 h en el servidor (el cliente no controla `expires_at`). `route_id` **sin FK** (al grabar la
  ruta es borrador local, aún no en `nk_routes`).
...
- [x] **Strava Fase 5 (Seguridad) — cliente** (2026-06-26): **PR1** check-in/S.O.S. por SMS (offline, validado en campo) + **PR2** seguimiento en vivo in-app + Supabase (`nk_live_sessions`, RLS solo-dueño + RPC `nk_get_live_session` SECURITY DEFINER por token, TTL 12 h por trigger; emisor sube cada ~10 s, visor = misma app con polling). Pasó `security-review`. **Requiere aplicar `supabase/schema.sql`.** **PR3 (visor web) hecho + validado 2026-06-28** (`404.astro` de la landing: Leaflet/OSM + polling del RPC con `anon`).
- [x] **Observabilidad** (2026-06-26): analítica in-house (`nk_events`) + reporte de bug (`nk_bug_reports`) + **dashboard Astro** (`D:\projects\nan-kamay-dashboard`). Ver `docs/ANALYTICS.md`.
- [x] **Lote 🟡 cliente** (2026-06-27): detalle **público con elevación interactiva**; **snap del planner a senderos OSM** (reusa `OsmPathsService` + `snapCoordsToReference`, reversible); **nombres de zona** vía `ReverseGeocodeService` (Nominatim, caché, fallback offline) + **desnivel por zona** (DEM-preciso cuando la ruta se ajustó con `refineElevationUseCase`, ya expuesto en detalle privado).
- [ ] **Pendientes (lista única y ordenada): ver `docs/STRAVA_ROADMAP.md` §"Pendientes por implementar"** — quedan: 🟢 **Fase 6 (social)**, visor web iOS Universal Links (falta Apple Team ID), y validación de campo (GPS reposo, auto-pausa). **Fase 5 PR3 (visor web en vivo) hecho 2026-06-28.**
